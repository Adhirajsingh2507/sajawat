...
| **1.8** | **B2B bulk enquiry + CRM + notifications (Model B)** ✅ DONE | **1.8a backend** (`commit 4abc328`): `CrmLead` model (stages new→contacted→follow_up→negotiation→won/lost, embedded staff notes, soft-delete); **gated, rate-limited `POST /enquiries`** → persist + **instant WhatsApp admin alert**; `NotificationProvider` abstraction + `WhatsAppProvider` (Meta Cloud API via fetch, **config-gated/dormant** — unconfigured sends skipped, never thrown); Settings singleton + `/admin/settings` (`SETTINGS_MANAGE`) holding the admin number; admin `/admin/crm/leads` (`CRM_READ`/`CRM_WRITE`). **1.8b frontend** (`commit ead87a7`): gated `/wholesale` enquiry form (web) + admin **CRM pipeline board** + lead detail (stage/assign/notes) + **Settings** page. **No automated wholesale checkout** (leads worked manually). Needs `WHATSAPP_*` keys + an admin number to alert live; leads persist regardless. | backend, ecommerce, frontend | — |
