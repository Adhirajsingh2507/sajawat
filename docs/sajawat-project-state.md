...
| **1.3b** | **Products + Inventory + Search** | `Product` (refs Category/Collections), `Inventory` (1:1) + `InventoryMovement` (audit); public list/filter/sort/search/featured/best-sellers/new-arrivals/by-slug; admin product CRUD + inventory adjust/history (RBAC). | backend, database, ecommerce | — |
