...
- **Current status (2026-06-15):** Phase 0 **complete**; **Phase 1 in progress — the entire backend commerce path is built** (catalog → cart → promotions → checkout → orders → admin order mgmt). Single-company two-model business (B2C retail + B2B bulk), **fully login-gated storefront** per owner decision (D17). `origin/develop` = `ad62081` (1.6a); 1.6b+1.6c committed locally ahead of it. **134 API tests** green; all gates green.
- **Done in Phase 1 (API unless noted):** **1.1** persistence + `BaseRepository` + `User` · **1.2** auth endpoints + session store + Google (closes D15) · **1.3a/b** catalog (categories, collections, products, inventory, search) · **1.4a** storefront foundation + design system + **login gate** (web), **1.4b** gated catalog pages (web) · **1.5** cart + wishlist + promotions engine · **1.6a** COD checkout + orders, **1.6b** Razorpay online (dormant until keys), **1.6c** admin order management. See §9 for the full table.
- **Remaining Phase 1:** storefront shopping UI (cart/wishlist/checkout/auth-UI pages — backend ready), **1.7** admin panel UI, **1.8** B2B leads + CRM + WhatsApp, **1.9** web CSP (D12) + security hardening, **1.10** launch readiness (e2e journeys, perf, monitoring, backups) → `v1.0.0`.
...
- **External deps to provide (build-complete, dormant until keys):** Google OAuth (`GOOGLE_CLIENT_ID`) for live Google sign-in (1.2/1.4); Razorpay (`RAZORPAY_KEY_ID`/`KEY_SECRET`/`WEBHOOK_SECRET`) for live online payments (1.6b) — COD works without keys.
