...
| **1.6b** | **Razorpay online + verify + webhook** | `Payment` model; `PaymentProvider` + `RazorpayProvider` behind config (`RAZORPAY_*`); `POST /checkout` (reserve stock + create gateway order), `POST /checkout/verify-payment` (HMAC verify → decrement → paid), `POST /webhooks/razorpay` (raw-body signature, **idempotent** source of truth). 501 until keys configured; mocked provider in tests. **Needs Razorpay keys from owner.** | ecommerce, backend, security | — |
